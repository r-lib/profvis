...
In addition to R code, you can also profile [Shiny](shiny.rstudio.com) applications. To do this, simply execute the `runApp()` command inside of `profvis`. For instance, we can run one of shiny's default examples using the `runExample` command (which is a wrapper for `runApp`).
