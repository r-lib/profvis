...
profvis({
  ...
  abline(m, col = "red")
}, height = "250px") # The height argument controls the display in this R Markdown document.
```

On the left is the code, and on the right is a flame graph. In the flame graph, the horizontal direction represents time in milliseconds, and the vertical direction represents the call stack. Looking at the bottom-most items on the stack, most of the time, about 2 seconds, is spent in `plot`, and then a much smaller amount of time is spent in `lm`, and almost no time at all is spent in `abline` -- it doesn't even show up on the flame graph.
...
p$x$message$split <- "v"
saveRDS(p, "vignettes/ggplot2.rds")
```
...
The profvis examples in this document have a vertical split, but by default, profvis visualizations have a horizontal split. In order to split them vertically, use `split="v"`, as in
